...
class Life {
    ...
    doEvent(eventId) {
        const { effect, next, description, postEvent, grade, choices } = this.#event.do(eventId);
    ...
    characterById(id) {
        const raw = this.#character.byId(id);
        if (!raw) return null;
        raw.talent = raw.talent.map(tid => this.#talent.get(tid));
        return raw;
    }
